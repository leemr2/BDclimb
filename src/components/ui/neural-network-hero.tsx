...
// SplitText is a Club GreenSock (paid) plugin; we use a simple fade/slide animation instead
...
export default function NeuralNetworkHero({
  ...
  useGSAP(
    () => {
      ...
      const runAnimation = () => {
        ...
        tl.to(headerRef.current, { autoAlpha: 1, y: 0, duration: 0.9 }, 0.1);
  ...
  return (
    <section ref={sectionRef} className="relative isolate min-h-screen w-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-24 bg-linear-to-t from-black/40 to-transparent" />
    </section>
  );
}
